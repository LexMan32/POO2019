    ...
    class Calcul
    {
        ...
        public Calcul(Random generateur, int nbrOperande, TypeOperation typeOperation)
        {
            ...
            if (this.typeOperation == TypeOperation.ALEATOIRE)
                this.typeOperation = (TypeOperation)generateur.Next(TypeOperation.GetValues(typeof(TypeOperation)).Cast<int>().Min(), TypeOperation.GetValues(typeof(TypeOperation)).Cast<int>().Max());
        }
